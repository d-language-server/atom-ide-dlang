...
class DLSLanguageClient extends alc.AutoLanguageClient {
  ...
  filterChangeWatchedFiles(filePath) {
    return ['dub.json', 'dub.sdl', 'dub.selections.json'].indexOf(path.basename(filePath)) !== -1
      ...
      || path.basename(filePath).endsWith('.d')
      || path.basename(filePath).endsWith('.di');
  }
  ...
  startServerProcess() {
    ...
    let dub = atom.config.get('ide-dlang.dubPath') || findInPath(executableName('dub'));
    let compiler = findInPath(executableName('dmd')) || findInPath(executableName('ldc'));
